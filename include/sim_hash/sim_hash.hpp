...
std::vector<uint64_t> get_hashed_tokens_v2(std::string &string, size_t window_size) {
    ...
    for(uint64_t i=0; i<(string.size()-window_size)+1; i++){
        tokens.push_back(string.substr(i, window_size));
        std::cout << "S " << i << " "<< string.substr(i, window_size) << std::endl;
    }
    std::vector<uint64_t> hashes;
    hashes.reserve(tokens.size());
    for(size_t i = 0; i < tokens.size(); ++i) {
...
uint64_t simhash64(std::string &string, size_t window_size) {
  ...
  std::vector<int> histogram(64, 0);
	...
	std::vector<uint64_t> hashes = get_hashed_tokens_v2(string, window_size);
